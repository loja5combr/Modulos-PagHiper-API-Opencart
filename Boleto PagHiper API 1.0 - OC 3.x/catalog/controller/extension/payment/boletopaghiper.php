...
class ControllerExtensionPaymentBoletoPagHiper extends Controller {
	...
	public function index() {
		$this->language->load('extension/payment/boletopaghiper');
		$order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        $data['id_pedido'] = $this->session->data['order_id'];
		$data['hash'] = md5(sha1($data['id_pedido']));
		$fiscal = '';
		$cpf = $this->config->get('payment_boletopaghiper_origem_cpf');
		$cnpj = $this->config->get('payment_boletopaghiper_origem_cnpj');
		...
		return $this->load->view('extension/payment/boletopaghiper', $data);
	}
	...
	public function ipn(){
        ...
        if(isset($_POST['transaction_id']) && $_POST['apiKey']==trim($this->config->get('payment_boletopaghiper_api_key'))){
	...
	private function post_pague(){
		...
		//custom
        $numero = $this->config->get('payment_boletopaghiper_origem_numero');
		$complemento = $this->config->get('payment_boletopaghiper_origem_complemento');
		...
		$json['notification_url'] = $this->url->link('extension/payment/boletopaghiper/ipn','','SSL');
		$json['discount_cents'] = number_format($this->getDescontos(), 2, '', '');
		$json['shipping_price_cents'] = number_format($this->getFrete(), 2, '', '');
		$json['days_due_date'] = (int)$this->config->get('payment_boletopaghiper_validade');
	...
	public function confirm() {
        ...
        if(($pague['status']==200 || $pague['status']==201) && isset($pague['retorno']['create_request']) && $pague['retorno']['create_request']['result']=='success'){
            ...
            $this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('payment_boletopaghiper_order_status_id'), $msg, true);
            $json['boleto'] = $id;
