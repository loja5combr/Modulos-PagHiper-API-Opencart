...
class ControllerPaymentBoletoPagHiper extends Controller {
	...
	public function index() {
		...
		if(isset($order_info[$cpf]) && !empty($order_info[$cpf])){
			$fiscal = preg_replace('/\D/', '', $order_info[$cpf]);	
		}elseif(isset($order_info[$cnpj]) && !empty($order_info[$cnpj])){
			$fiscal = preg_replace('/\D/', '', $order_info[$cnpj]);	
		}
		$data['continue'] = $this->url->link('checkout/success','','SSL');
		$data['fiscal'] = $fiscal;

		$this->data = $data;
        $this->template = 'default/template/payment/boletopaghiper.tpl';
		$this->render();
	}
	...
	public function ipn(){
        ...
        if(isset($_POST['transaction_id']) && $_POST['apiKey']==trim($this->config->get('boletopaghiper_api_key'))){
	...
	public function confirm() {
        ...
        if(($pague['status']==200 || $pague['status']==201) && isset($pague['retorno']['create_request']) && $pague['retorno']['create_request']['result']=='success'){
            ...
            $this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('boletopaghiper_order_status_id'), $msg, true);
            $json['boleto'] = $id;
